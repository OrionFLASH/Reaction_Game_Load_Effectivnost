...
1. **Загрузка данных**: Читает Excel файлы из папки INPUT
2. **Обработка данных**: 
   - Объединяет данные из всех файлов
   - Удаляет дубликаты
   - Заполняет пропущенные значения
   - Сортирует по первому столбцу
3. **Сохранение результатов**: Создает файлы в форматах CSV и Excel в папке OUTPUT
